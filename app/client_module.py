...
from .config import VIDEO_PORT, COMMAND_PORT, BUFFER_SIZE, DEFAULT_WIDTH, DEFAULT_HEIGHT, DISCOVERY_PORT
import logging
import os
...
class DiscoveryScanner(QObject):
    """Scanner pour découvrir les serveurs de partage d'écran sur le réseau local"""
    
    server_found = Signal(dict)  # Émet {name, ip, port, video_port}
    ...
    def start_scan(self, duration=3.0):
        """Lance un scan pendant `duration` secondes"""
        if self._running:
            return
